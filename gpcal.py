...
# Uncomment these lines if you want to make your IERS file up-to-date.
...
class polcal(object):
    def __init__(self, aips_userno, scriptpath, direc, dataname, calsour, source, cnum, autoccedt, Dbound = 1.0, Pbound = np.inf, outputname = None, drange = None, \
                 selfcal=False, solint = None, solmode = None, soltype = None, weightit = None, zblcalsour = None, zblant = None, zblcal = False, \
                 fixdterm = False, pol_fixdterm = False, fixant = None, fixdr = None, fixdl = None, transferdterm = False, transferdtermant = None, \
        ...
        self.scriptpath = copy.deepcopy(scriptpath)
        if(self.scriptpath[-1] != '/'): self.scriptpath = self.scriptpath + '/'
        ...
        self.colors = ['blue', 'red', 'orange', 'steelblue', 'green', 'slategrey', 'cyan', 'royalblue', 'purple', 'blueviolet', 'darkcyan', 'darkkhaki', 'magenta', 'navajowhite', 'darkred'] * 5

        self.markerarr = ['o', '^', 's', '<', 'p', '*', 'X', 'P', 'D', 'v', 'd', 'x']
        ...
        for i in range(len(self.calsour)):
            if not path.exists(self.direc + self.dataname + self.calsour[i] + '.uvf'):
                raise Exception("{:s} does not exist in {:s}.".format(self.dataname + self.calsour[i] + '.uvf', self.direc + 'gpcal/'))
            
            if not path.exists(self.direc + self.dataname + self.calsour[i] + '.fits'):
                raise Exception("{:s} does not exist in {:s}.".format(self.dataname + self.calsour[i] + '.fits', self.direc + 'gpcal/'))
            
            if(self.autoccedt[i] == False):
                if not path.exists(self.direc + 'gpcal/' + self.dataname + self.calsour[i] + '.box'):
        ...
        for i in range(len(self.polcalsour)):
            if not path.exists(self.direc + self.dataname + self.polcalsour[i] + '.uvf'):
                raise Exception("{:s} does not exist in {:s}.".format(self.dataname + self.polcalsour[i] + '.uvf', self.direc + 'gpcal/'))
            
            if not path.exists(self.direc + self.dataname + self.polcalsour[i] + '.fits'):
                raise Exception("{:s} does not exist in {:s}.".format(self.dataname + self.polcalsour[i] + '.fits', self.direc + 'gpcal/'))
                                
            
    
    def runfitld(self, outname, outclass, infile):    
    ...
    def cleanqu(self, source, data, mask, save, log):
        ...
        if not path.exists(self.direc+'GPCAL_Difmap_v1'):
            self.logger.info('The Difmap script does not exist in the working directory. Will copy it from the script folder.')
            os.system('cp ' + self.scriptpath+'GPCAL_Difmap_v1 ' + self.direc)
        ...
        for spoliter in range(self.selfpoliter):
            ...
            for l in range(len(self.polcalsour)):
                ...
                if(spoliter == 0): 
                    ...
                    self.cleanqu(self.polcalsour[l], self.dataname+self.polcalsour[l]+'.dtcal.uvf', self.dataname+self.polcalsour[l]+'.win', self.dataname+self.polcalsour[l], difmaplogfile)
                    if self.difmaplog:
                    ...
                    self.logger.info('uvfits file: {:s}, CLEAN mask: {:s}, save file: {:s}\n'.format(self.dataname+'pol.iter'+str(spoliter)+'.'+self.polcalsour[l]+'.dtcal.uvf', self.dataname+self.polcalsour[l]+'.win', \
                                     self.dataname+self.polcalsour[l]+'.IF*.q,u'))
                    self.cleanqu(self.polcalsour[l], self.dataname+'pol.iter'+str(spoliter)+'.'+self.polcalsour[l]+'.dtcal.uvf', self.dataname+self.polcalsour[l]+'.win', self.dataname+self.polcalsour[l], difmaplogfile)
                    if self.difmaplog:
                        os.system('cat ' + self.logfile + ' ' + difmaplogfile + ' > ' + self.direc+'gpcal/'+'temp.log')
                        os.system('mv ' +self.direc+'gpcal/'+'temp.log ' + self.logfile)
